...
async function getSessionList(page) {
  ...
  const initialSessions = await page.evaluate(strategy.fn);
  ...
  if (initialSessions.length > 0) {
    const clicked = await page.evaluate((href) => {
      const link = Array.from(document.querySelectorAll('a[href*="sessionId="]'))
      ...
      link.click();
      return true;
    }, initialSessions[0].href);

    if (clicked) {
      await waitForStable(page);

      const subsequentSessions = await page.evaluate(strategy.fn);
      subsequentSessions.forEach(session => {
        if (session?.sessionId && !sessionsById.has(session.sessionId)) {
          sessionsById.set(session.sessionId, session);
        }
      });
    }
